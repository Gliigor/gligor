...
import { loadCategories, categoryColor } from "./sharedCategories";

// ─── Types ────────────────────────────────────────────────────────────────────
interface IncomeRow  { id: string; label: string; amount: string; }
interface ExpenseRow { id: string; label: string; amount: string; }
interface SavedBudget { income: IncomeRow[]; expenses: ExpenseRow[]; savedAt: string; }
...
// ─── Default expense rows (aligned with shared categories) ───────────────────
const makeDefaults = (): ExpenseRow[] => [
  { id:"e1",  label:"Housing",         amount:"" },
...
// ─── Styles ───────────────────────────────────────────────────────────────────
const IS: React.CSSProperties = { background:"rgba(0,0,0,0.35)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:8, padding:"9px 13px", color:"hsl(var(--foreground))", fontFamily:"'Exo 2',sans-serif", fontSize:13, outline:"none", transition:"border-color 0.2s", width:"100%" };
const labelIS: React.CSSProperties = { ...IS, flex:1 };
const amountIS: React.CSSProperties = { ...IS, width:130, textAlign:"right", flexShrink:0 };
const delBtn: React.CSSProperties = { background:"rgba(248,113,113,0.08)", border:"1px solid rgba(248,113,113,0.15)", borderRadius:6, padding:"8px 12px", color:"#f87171", fontSize:13, cursor:"pointer", flexShrink:0 };
const sectionS: React.CSSProperties = { border:"1px solid rgba(255,255,255,0.07)", borderRadius:14, padding:"28px 32px", background:"rgba(255,255,255,0.015)", backdropFilter:"blur(8px)", marginBottom:24 };

const AnimRow = ({ children, visible }: { children: React.ReactNode; visible: boolean }) => (
  <div style={{ display:"flex", gap:10, alignItems:"center", opacity:visible?1:0, transform:visible?"translateY(0)":"translateY(-8px)", transition:"opacity 0.25s ease,transform 0.25s ease", marginBottom:10 }}>
    {children}
  </div>
);
              ...
              {rows.map((r)=>{
                ...
                return (
                  <tr key={r.category} style={{ borderBottom:"1px solid rgba(255,255,255,0.03)" }}>
                    ...
                    <td style={{ padding:"11px 10px" }}>
                      <span style={{ fontSize:10, padding:"3px 8px", borderRadius:20, background:b.bg, color:b.color }}>{b.label}</span>
                    </td>
                  </tr>
                );
              })}
...
export default function BudgetMaker() {
  ...
  const [expenses,    setExpenses]    = useState<ExpenseRow[]>(makeDefaults());
  ...
  // Animate rows visible on mount
  useEffect(() => {
    const vis: Record<string,boolean> = {};
    [...income, ...expenses].forEach((r) => { vis[r.id] = true; });
    setRowVis(vis);
  }, []);

  // Sync shared categories when another tab changes them
  useEffect(() => {
  ...
  const addExpense = () => {
    const id = uid();
    setExpenses((p)=>[...p,{id,label:"",amount:""}]);
    setTimeout(()=>setRowVis((v)=>({...v,[id]:true})),10);
  };
  ...
  const saveBudget = useCallback(() => {
    try {
      ...
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ income, expenses, savedAt:now }));
      setSavedAt(now); setSaveMsg("Saved!"); setTimeout(()=>setSaveMsg(""),2000);
    } catch {}
  },[income,expenses]);
  ...
  const buildRows = (): BudgetComparison[] => {
    // budget amounts per category label
    const budgetMap: Record<string,number> = {};
    for (const row of expenses) {
      const amt = parseAmt(row.amount);
      if (amt > 0) budgetMap[row.label] = amt;
    }
    // actual spending from CSV localStorage
  ...
  return (
    <div className="min-h-screen bg-background relative" style={{ fontFamily:"'Exo 2',sans-serif" }}>
      ...
      {/* ── Sticky Summary Bar ─────────────────────────────────────────────── */}
      <div style={{ position:"sticky", top:0, zIndex:50, background:"rgba(5,8,18,0.92)", backdropFilter:"blur(16px)", borderBottom:"1px solid rgba(0,229,255,0.1)", padding:"12px 24px" }}>
        <div style={{ maxWidth:900, margin:"0 auto" }}>
          ...
          <div style={{ height:4, background:"rgba(255,255,255,0.05)", borderRadius:2, overflow:"hidden" }}>
            <div style={{ height:"100%", width:`${allocPct}%`, background:allocPct>95?"#fb923c":allocPct>80?"#fbbf24":"#00e5ff", borderRadius:2, transition:"width 0.4s ease,background 0.4s ease" }} />
          </div>
          <div style={{ display:"flex", justifyContent:"space-between", marginTop:4 }}>
            <span style={{ fontSize:9, color:"#334155", letterSpacing:"0.1em" }}>0%</span>
            <span style={{ fontSize:9, color:allocPct>95?"#fb923c":"#334155", letterSpacing:"0.1em", transition:"color 0.3s" }}>{allocPct.toFixed(0)}% ALLOCATED</span>
      ...
      <main className="relative z-10" style={{ maxWidth:900, margin:"0 auto", padding:"48px 24px 80px" }}>
        ...
        <div style={sectionS}>
          ...
          <div style={{ display:"flex", gap:10, marginBottom:8, paddingRight:46 }}>
            <span style={{ fontSize:10, letterSpacing:"0.2em", color:"#475569", fontFamily:"'Orbitron',sans-serif", flex:1 }}>SOURCE</span>
            <span style={{ fontSize:10, letterSpacing:"0.2em", color:"#475569", fontFamily:"'Orbitron',sans-serif", width:130, textAlign:"right", flexShrink:0 }}>MONTHLY (€)</span>
          </div>
          {income.map((r)=>(
            <AnimRow key={r.id} visible={rowVis[r.id]??true}>
              <input type="text" placeholder="e.g. Salary, Freelance…" value={r.label} onChange={(e)=>setIncome((p)=>p.map((x)=>x.id===r.id?{...x,label:e.target.value}:x))} style={labelIS} onFocus={fb} onBlur={bb} />
              <input type="number" placeholder="0.00" value={r.amount} onChange={(e)=>setIncome((p)=>p.map((x)=>x.id===r.id?{...x,amount:e.target.value}:x))} style={amountIS} onFocus={fb} onBlur={bb} />
              <button onClick={()=>removeIncome(r.id)} style={delBtn}>✕</button>
            </AnimRow>
          ))}
          <button onClick={addIncome} style={{ marginTop:8, background:"none", border:"1px dashed rgba(74,222,128,0.25)", borderRadius:8, padding:"9px 18px", color:"#4ade80", fontFamily:"'Orbitron',sans-serif", fontSize:10, letterSpacing:"0.12em", cursor:"pointer", width:"100%", transition:"all 0.2s" }}
        ...
        <div style={sectionS}>
          ...
          <div style={{ display:"flex", gap:10, marginBottom:8, paddingRight:46 }}>
            <span style={{ fontSize:10, letterSpacing:"0.2em", color:"#475569", fontFamily:"'Orbitron',sans-serif", flex:1 }}>CATEGORY</span>
            <span style={{ fontSize:10, letterSpacing:"0.2em", color:"#475569", fontFamily:"'Orbitron',sans-serif", width:130, textAlign:"right", flexShrink:0 }}>MONTHLY (€)</span>
          </div>
          {expenses.map((r)=>{
            ...
            return (
              <AnimRow key={r.id} visible={rowVis[r.id]??true}>
                <div style={{ flex:1, position:"relative" }}>
                  <input type="text" placeholder="Category name…" value={r.label} onChange={(e)=>setExpenses((p)=>p.map((x)=>x.id===r.id?{...x,label:e.target.value}:x))} style={{ ...labelIS, paddingRight:amt>0?60:13 }} onFocus={fb} onBlur={bb} />
                  {amt>0&&<span style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", fontSize:10, color:"#334155", pointerEvents:"none" }}>{pct.toFixed(0)}%</span>}
                </div>
                <input type="number" placeholder="0.00" value={r.amount} onChange={(e)=>setExpenses((p)=>p.map((x)=>x.id===r.id?{...x,amount:e.target.value}:x))} style={{ ...amountIS, color:amt>0?"#f87171":"hsl(var(--muted-foreground))" }} onFocus={fb} onBlur={bb} />
                <button onClick={()=>removeExpense(r.id)} style={delBtn}>✕</button>
              </AnimRow>
            );
          })}
          <button onClick={addExpense} style={{ marginTop:8, background:"none", border:"1px dashed rgba(248,113,113,0.25)", borderRadius:8, padding:"9px 18px", color:"#f87171", fontFamily:"'Orbitron',sans-serif", fontSize:10, letterSpacing:"0.12em", cursor:"pointer", width:"100%", transition:"all 0.2s" }}
        ...
        <div style={{ ...sectionS, borderColor:remaining>=0?"rgba(0,229,255,0.15)":"rgba(251,146,60,0.2)" }}>
          ...
          <div>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
              ...
              <span style={{ fontSize:10, color:allocPct>100?"#fb923c":"#64748b", letterSpacing:"0.08em" }}>{allocPct.toFixed(1)}% of income</span>
            </div>
            <div style={{ height:8, background:"rgba(255,255,255,0.05)", borderRadius:4, overflow:"hidden" }}>
              <div style={{ height:"100%", width:`${Math.min(allocPct,100)}%`, background:allocPct>100?"#fb923c":allocPct>85?"#fbbf24":"linear-gradient(90deg,#00e5ff,#4ade80)", borderRadius:4, transition:"width 0.5s ease,background 0.4s" }} />
            </div>
            {totalExpenses>0&&(
              <div style={{ display:"flex", height:3, marginTop:4, borderRadius:2, overflow:"hidden", gap:1 }}>
                {expenses.filter((r)=>parseAmt(r.amount)>0).map((r,i)=>{
                  const colors=["#60a5fa","#4ade80","#f472b6","#fb923c","#a78bfa","#34d399","#fbbf24","#f87171","#94a3b8","#00e5ff","#e879f9","#22d3ee","#86efac","#fca5a5","#c4b5fd"];
                  return <div key={r.id} style={{ flex:`0 0 ${(parseAmt(r.amount)/totalExpenses)*100}%`, background:colors[i%colors.length] }} title={r.label} />;
                })}
              </div>
            )}
          </div>
          {remaining<0&&<div style={{ marginTop:16, padding:"12px 16px", background:"rgba(251,146,60,0.08)", border:"1px solid rgba(251,146,60,0.2)", borderRadius:8, fontSize:12, color:"#fb923c" }}>⚠ You are over budget by <strong>{fmt(Math.abs(remaining))}</strong>. Consider reducing some categories.</div>}
          {remaining>0&&totalIncome>0&&<div style={{ marginTop:16, padding:"12px 16px", background:"rgba(0,229,255,0.05)", border:"1px solid rgba(0,229,255,0.15)", borderRadius:8, fontSize:12, color:"#00e5ff" }}>✓ {fmt(remaining)} unallocated — consider adding it to Savings or Investments.</div>}
        </div>
        ...
        <div style={{ display:"flex", gap:12, justifyContent:"flex-end", flexWrap:"wrap" }}>
          ...
          <button onClick={()=>setShowCompare(true)} style={{ background:"rgba(251,191,36,0.08)", border:"1px solid rgba(251,191,36,0.3)", borderRadius:8, padding:"11px 24px", color:"#fbbf24", fontFamily:"'Orbitron',sans-serif", fontSize:10, letterSpacing:"0.15em", cursor:"pointer", transition:"all 0.2s" }}
            onMouseEnter={(e)=>e.currentTarget.style.background="rgba(251,191,36,0.15)"}
            onMouseLeave={(e)=>e.currentTarget.style.background="rgba(251,191,36,0.08)"}>
            ⚖️ COMPARE WITH CSV
          </button>
          <button onClick={saveBudget} style={{ background:"rgba(0,229,255,0.08)", border:"1px solid rgba(0,229,255,0.3)", borderRadius:8, padding:"11px 24px", color:"#00e5ff", fontFamily:"'Orbitron',sans-serif", fontSize:10, letterSpacing:"0.15em", cursor:"pointer", transition:"all 0.2s" }}
            onMouseEnter={(e)=>e.currentTarget.style.background="rgba(0,229,255,0.15)"}
            onMouseLeave={(e)=>e.currentTarget.style.background="rgba(0,229,255,0.08)"}>
            💾 SAVE BUDGET
          </button>
          <button onClick={resetBudget} style={{ background:"rgba(248,113,113,0.06)", border:"1px solid rgba(248,113,113,0.2)", borderRadius:8, padding:"11px 24px", color:"#f87171", fontFamily:"'Orbitron',sans-serif", fontSize:10, letterSpacing:"0.15em", cursor:"pointer", transition:"all 0.2s" }}
